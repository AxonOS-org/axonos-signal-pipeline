...
| **v0.8.0** (met) | Calibration: covariance, session mean, drift update, Cholesky whitening, ZeroCalib skeleton | L1: `covariance` / `whiten_cholesky` vectors reproduced; `align(W,R)` pinned as the `Q16` identity to fixed-point error | A calibration step that is not a pure function of its declared inputs |
